...
8. Write the result to root `winutil.ps1`, without comments (except the first, the header), indentation or blank lines outside strings (`Compress-WinUtilScript`).

Config JSON is embedded compact (`ConvertTo-Json -Compress`) and XAML without comments, indentation or blank lines (`Compress-WinUtilXaml`). Both are read as UTF-8, and every non-ASCII character is escaped so the compiled script is pure ASCII: Windows PowerShell 5.1 and `irm | iex` decode scripts with a legacy code page, which would otherwise corrupt the Arabic interface text. Arabic letters (U+0600-U+06FF) in a config or the XAML become `~` plus two hex digits (any literal `~` is escaped first), decoded by a `[regex]::Replace` line right after the here-string; everything else non-ASCII becomes `\uXXXX` in JSON and `&#xXXXX;` in XAML.
...
- Theme, package manager, language and font size persist in `%LocalAppData%\winutil\preferences.json` (`Save-WinUtilPreferences` / `Import-WinUtilPreferences`); the default theme is Dark and the default language is Arabic (`ar`).
- English mode (`language` = `en`, toggled from the Settings menu's `LanguageMenuItem`, applied on the next launch): `Initialize-WinUtilTranslation` keeps only the `english` section of `translations.json`, so configs and `Get-WinUtilText` defaults stay English, and `Convert-WinUtilXamlToEnglish` replaces the window XAML's Arabic text through that section's Arabic-to-English map and lays the window out left to right. Every Arabic text added to the XAML needs an `english` entry; a Pester test enforces it.
- Tweaks may carry `"RestartRequired": "true"`; after the tweaks or undo job, `Show-WinUtilRestartNotice` lists the ones marked so.
- The Gaming tab's startup programs card (`Get-WinUtilStartupApps`, `Set-WinUtilStartupApp`) toggles Run entries and Startup-folder shortcuts through the `Explorer\StartupApproved` records Task Manager uses (they are never deleted), Microsoft Store apps through their startup task `State` value, and sign-in scheduled tasks outside `\Microsoft\` (`Get-WinUtilStartupTasks`) through Enable/Disable-ScheduledTask. The game server latency test measures every region at once (`Measure-WinUtilGameServerLatency`). The update banner's button replaces the running copy with the latest release (`Start-WinUtilSelfUpdate`). The Temperatures tab (`WPFTab9`, header `Temps`) polls `nvidia-smi` (`Get-WinUtilGpuSensor`) and the ACPI thermal zone counters plus processor load (`Get-WinUtilCpuSensor`) every three seconds, only while it is the current tab (`Start-WinUtilTemperatureMonitor`); Memory use and speed (`Get-WinUtilMemorySensor`) refresh on the same tick; disk health, temperature, wear and power-on hours (`Get-WinUtilDiskHealth`, `MSFT_PhysicalDisk` and `MSFT_StorageReliabilityCounter` through CIM) every 30 seconds. `Format-WinUtilTemperatureReading` turns the readings into its texts and green/orange/red levels; the last two minutes of GPU and CPU temperature are drawn under the numbers, and `Show-WinUtilHeatAlert` raises a Windows notification (at most every five minutes) when either runs hot. The timer stops when the window closes. The drive speed button runs `winsat disk` on the system drive (`Measure-WinUtilDiskSpeed`, `Start-WinUtilDiskSpeedTest`).
