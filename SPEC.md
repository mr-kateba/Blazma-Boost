...
- The Gaming tab's startup programs card (`Get-WinUtilStartupApps`, `Set-WinUtilStartupApp`) toggles entries through the `Explorer\StartupApproved` records Task Manager uses; Run entries and Startup-folder shortcuts are never deleted. The Temperatures tab (`WPFTab9`, header `Temps`) polls `nvidia-smi` (`Get-WinUtilGpuSensor`) and the ACPI thermal zone counters plus processor load (`Get-WinUtilCpuSensor`) every three seconds, only while it is the current tab (`Start-WinUtilTemperatureMonitor`); Memory use and speed (`Get-WinUtilMemorySensor`) refresh on the same tick; disk health, temperature, wear and power-on hours (`Get-WinUtilDiskHealth`, Storage module SMART counters) every 30 seconds. `Format-WinUtilTemperatureReading` turns the readings into its texts and green/orange/red levels. The drive speed button runs `winsat disk` on the system drive (`Measure-WinUtilDiskSpeed`, `Start-WinUtilDiskSpeedTest`).
